...
locals {
  ...
  catalog_data = jsondecode(file("${path.module}/../../environments/staging/config/settings.json"))
...
# 5. Storing terraform state in local for staging enviornment #########################
terraform {
  backend "local" {
...
output "frontend_url" {
  ...
  value       = "http://localhost:${local.catalog_data.frontend.external_ports.staging}"
}
