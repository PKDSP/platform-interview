...
locals {
  ...
  catalog_data = jsondecode(file("${path.module}/../../environments/production/config/settings.json"))
...
output "vault_address" {
  description = "The address of the Vault server for the production environment."
  value       = local.catalog_data.vault.address
}

output "frontend_url" {
  description = "The URL to access the frontend application."
  value       = "http://localhost:${local.catalog_data.frontend.external_ports.production}"
}
